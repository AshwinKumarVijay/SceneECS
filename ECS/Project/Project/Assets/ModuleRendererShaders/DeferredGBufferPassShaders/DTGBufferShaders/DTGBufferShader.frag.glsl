...
layout (location = 3) in vec2 v_vertexTextureCoordinates1;
layout (location = 4) in vec2 v_vertexTextureCoordinates2;
...
layout (binding = 15) uniform sampler2D u_diffuseAlbedoTexture;
...
void main(void)
{
	...
	vec3 diffuseAlbedo = u_diffuseAlbedoAndLitType.xyz * (texture(u_diffuseAlbedoTexture, v_vertexTextureCoordinates1)).xyz;
	o_diffuseAlbedoAndLitType = vec4(u_diffuseAlbedoAndLitType.xyz * diffuseAlbedo, u_diffuseAlbedoAndLitType.w);
